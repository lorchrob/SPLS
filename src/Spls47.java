...
class Spls47 {
  ...
  public static void initializePath(int[][] path) {
    ...
    // add small groups of squares
    k = initializePathHelper(path, 7, 7, 7, 7, k, 0);

    k = initializePathHelper(path, 7, 20, 7, 20, k, 0);

    k = initializePathHelper(path, 20, 7, 20, 7, k, 0);

    k = initializePathHelper(path, 20, 20, 20, 20, k, 0);

    k = initializePathHelper(path, 12, 12, 15, 15, k, 0);

    k = initializePathHelper(path, 12, 7, 15, 7, k, 0);

    k = initializePathHelper(path, 12, 20, 15, 20, k, 0);

    k = initializePathHelper(path, 7, 12, 7, 15, k, 0);

    k = initializePathHelper(path, 20, 12, 20, 15, k, 0);

    // add top rows (not all slots)
    // add F_1 and F_2
    k = initializePathHelper(path, 4, 12, 6, 15, k, 0);

    // add H_2 and D_2
    k = initializePathHelper(path, 4, 20, 6, 23, k, 0);

    // add C_1
    k = initializePathHelper(path, 4, 0, 6, 3, k, 24);

    // add C_2
    k = initializePathHelper(path, 4, 8, 6, 11, k, 0);

    // add E_1
    k = initializePathHelper(path, 0, 12, 3, 13, k, 2);

    // add C_3
    k = initializePathHelper(path, 4, 16, 6, 19, k, 0);

    // add E_2
    k = initializePathHelper(path, 0, 14, 3, 15, k, 4);

    // add G_2
    k = initializePathHelper(path, 0, 20, 3, 20, k, 4);

    // add C_4
    k = initializePathHelper(path, 4, 24, 6, 27, k, 0);

    // add B_2
    k = initializePathHelper(path, 0, 21, 3, 23, k, 5);

    // add first rotation of top rows
    k = initializePathHelper(path, 7, 21, 7, 23, k, 0);

    k = initializePathHelper(path, 12, 21, 15, 23, k, 0);

    k = initializePathHelper(path, 20, 21, 23, 23, k, 0);

    k = initializePathHelper(path, 8, 21, 11, 23, k, 0);

    k = initializePathHelper(path, 7, 24, 7, 27, k, 7);

    k = initializePathHelper(path, 12, 24, 13, 27, k, 7);

    k = initializePathHelper(path, 16, 21, 19, 23, k, 0);

    k = initializePathHelper(path, 14, 24, 15, 27, k, 9);

    k = initializePathHelper(path, 20, 24, 20, 27, k, 9);

    k = initializePathHelper(path, 24, 21, 27, 23, k, 0);
    
    k = initializePathHelper(path, 21, 24, 23, 27, k, 10);

    // add second rotation of top rows
    k = initializePathHelper(path, 21, 20, 23, 20, k, 0);

    k = initializePathHelper(path, 21, 12, 23, 15, k, 0);

    k = initializePathHelper(path, 21, 4, 23, 7, k, 0);

    k = initializePathHelper(path, 21, 16, 23, 19, k, 0);

    k = initializePathHelper(path, 24, 20, 27, 20, k, 12);

    k = initializePathHelper(path, 24, 14, 27, 15, k, 12);

    k = initializePathHelper(path, 21, 8, 23, 11, k, 0);

    k = initializePathHelper(path, 24, 12, 27, 13, k, 14);

    k = initializePathHelper(path, 24, 7, 27, 7, k, 14);

    k = initializePathHelper(path, 21, 0, 23, 3, k, 0);

    k = initializePathHelper(path, 24, 4, 27, 6, k, 15);

    // add third (and final) rotation of top rows
    k = initializePathHelper(path, 20, 4, 20, 6, k, 0);

    k = initializePathHelper(path, 12, 4, 15, 6, k, 0);

    k = initializePathHelper(path, 7, 4, 7, 6, k, 0);

    k = initializePathHelper(path, 16, 4, 19, 6, k, 0);

    k = initializePathHelper(path, 20, 0, 20, 3, k, 17);

    k = initializePathHelper(path, 14, 0, 15, 3, k, 17);

    k = initializePathHelper(path, 8, 4, 11, 6, k, 0);
   
    k = initializePathHelper(path, 12, 0, 13, 3, k, 19);

    k = initializePathHelper(path, 7, 0, 7, 3, k, 19);

    // add middle region (not all slots)
    // add D_1 through D_4
    k = initializePathHelper(path, 7, 8, 7, 11, k, 0);

    k = initializePathHelper(path, 7, 16, 7, 19, k, 0);

    k = initializePathHelper(path, 20, 8, 20, 11, k, 0);

    k = initializePathHelper(path, 20, 16, 20, 19, k, 0);

    // add C_1 through C_4
    k = initializePathHelper(path, 12, 8, 13, 11, k, 0);

    k = initializePathHelper(path, 14, 8, 15, 11, k, 0);

    k = initializePathHelper(path, 12, 16, 13, 19, k, 0);

    k = initializePathHelper(path, 14, 16, 15, 19, k, 0);

    // add E_1 through E_4
    k = initializePathHelper(path, 8, 7, 11, 7, k, 20);

    k = initializePathHelper(path, 8, 20, 11, 20, k, 21);

    k = initializePathHelper(path, 16, 7, 19, 7, k, 22);

    k = initializePathHelper(path, 16, 20, 19, 20, k, 23);

    // add B_1 through B_4
    k = initializePathHelper(path, 8, 12, 11, 13, k, 20);

    k = initializePathHelper(path, 8, 14, 11, 15, k, 21);

    k = initializePathHelper(path, 16, 12, 19, 13, k, 22);

    k = initializePathHelper(path, 16, 14, 19, 15, k, 23);

    // add 4x4 squares
    k = initializePathHelper(path, 8, 8, 11, 11, k, 0);

    k = initializePathHelper(path, 8, 16, 11, 19, k, 0);

    k = initializePathHelper(path, 16, 8, 19, 11, k, 0);

    k = initializePathHelper(path, 16, 16, 19, 19, k, 0);

    k = initializePathHelper(path, 0, 0, 3, 3, k, 0);

    k = initializePathHelper(path, 0, 8, 3, 11, k, 0);

    k = initializePathHelper(path, 0, 16, 3, 19, k, 0);

    k = initializePathHelper(path, 0, 24, 3, 27, k, 0);

    k = initializePathHelper(path, 8, 0, 11, 3, k, 0);

    k = initializePathHelper(path, 8, 24, 11, 27, k, 0);

    k = initializePathHelper(path, 16, 0, 19, 3, k, 0);

    k = initializePathHelper(path, 16, 24, 19, 27, k, 0);

    k = initializePathHelper(path, 24, 0, 27, 3, k, 0);

    k = initializePathHelper(path, 24, 8, 27, 11, k, 0);

    k = initializePathHelper(path, 24, 16, 27, 19, k, 0);

    initializePathHelper(path, 24, 24, 27, 27, k, 0);
  }
  ...
  public static boolean fill2dArraySection(Integer[][] arr, int[][] path, int start, int finish,
    ...
    while (current <= finish && i <= 20000000) {
      try {
  ...
  public static void main(String[] args) {
    ...
    int[][] path = new int[(28 * 28) - 28][3];

    // keep track of the previous values for each element
    ArrayList<ArrayList<Integer>> previousValues = new ArrayList<>((28 * 28) - 28);
